...
> babel 的 parser 是基于 `acorn` 扩展而来的，而 `acorn` 也不是最早的 js parser
...
![](../.vuepress/public/images/babel/0021.png) 
...
其实这些可以在 [babel parser](https://babeljs.io/docs/en/babel-parser#output) 的文档里看到：
...
* ImportExpression 替换为了 CallExpression 并且 callee 属性设置为 Import 等
...
这样 acorn 就会在 parse 的时候分出 `guang` 这个关键字了
...
基于火狐浏览器的 JS 引擎 SpiderMonkey 的 AST 标准，制定了 espree 的标准，最早的 estree 标准的实现是 esprima，但是随着 es2015 开始一年一个版本，esprima 的迭代速度逐渐跟不上了，这时候 acorn 流行起来，因为速度更快，而且支持插件扩展，于是 espree、babel parser(babylon) 等都基于 acorn 来实现各自的 parser。虽然 estree 系列的 js parser 挺多的，但也不是全部，terser、typescript 等都是用自己的 AST。
